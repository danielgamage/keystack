...
export const processMIDI = {
  Transpose: (notes, newNotes, effect, effectIndex, effectsArray, oldState) => {
    ...
    nextInMIDIChain(value, newNotes, effect, effectIndex, effectsArray, oldState)
  },
      ...
      return [...new Set(effect.value)].map(tone => {
        ...
        if (effect.inversionChance > 0) {
          if (getRandomBoolean(effect.inversionChance)) {
            const multiplier = getRandomIntegerInRange(effect.inversionRange * -1, effect.inversionRange)
  ...
  DisableNotes: (notes, newNotes, effect, effectIndex, effectsArray, oldState) => {
...
export const nextInMIDIChain = (notes, newNotes, effect, effectIndex, effectsArray, oldState) => {
  if ((effectsArray.length - 1) !== effectIndex) {
    const nextIndex = effectIndex + 1
    processMIDI[effectsArray[nextIndex].devicePrototype]([...new Set(notes)], newNotes, effectsArray[nextIndex], nextIndex, effectsArray, oldState)
  } else {
...
export const startMIDIChain = (oldState) => {
  ...
  const newNotes = notes.filter(note => (
    !oldState.notes.input.some(oldNote => oldNote.index === note.index)
  ))
  const midiDevices = getDevicesOfType(state, state.tracks[0].devices, 'midi')
  if (midiDevices.length > 0) {
    processMIDI[midiDevices[0].devicePrototype](notes, newNotes, midiDevices[0], 0, midiDevices, oldState)
  } else {

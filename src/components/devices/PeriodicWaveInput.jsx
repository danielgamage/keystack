...
class Waveform extends Component {
  ...
  generateWave(type) {
    ...
    switch (type) {
      case "sine":
        ...
        break
      case "saw":
        waveArray.map((el, i) => 1 / ((i + 1) * Math.PI))
      case "square":
        waveArray.map((el, i) =>
          this.matchesSelection(i, "odd") ? 1 / ((i + 1) * Math.PI) : 0
        )
      default:
    }
  }
  ...
  setWaveform(type) {
    this.props.onInput(this.generateWave(type))
  }
  ...
  setScales() {
    this.y = scaleLinear().domain([0, 1]).rangeRound([viewBoxHeight, 0])

    this.x = scaleBand()
      .domain(
        Array(this.props.resolution)
          .fill()
          .map((el, i) => i)
      )
  ...
  setBackground() {
    this.backgrounds = Array(this.props.resolution)
      .fill()
      .map((rect, i) => (
        <rect

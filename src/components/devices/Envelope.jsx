...
class Envelope extends Component {
  ...
  render() {
    ...
    const pointsDict = {
      ...
      releaseEnd: {
        x:
          log10(envelope.attack) +
          log10(envelope.decay) +
          sustainWidth +
          log10(envelope.release),
        y: 0,
      },
    }
    ...
    const lines = [
      ...
      [pointsDict.sustainStart, pointsDict.sustainEnd],
      [
        pointsDict.sustainEnd,
        getControlPointForBias(
          pointsDict.sustainEnd,
          pointsDict.releaseEnd,
          envelope.releaseBias
        ),
        pointsDict.releaseEnd,
      ],
    ]

    const points = Object.keys(pointsDict).map((key) => pointsDict[key])
    ...
    const y = scaleLinear().domain([0, 1]).range([viewBoxHeight, 0])
